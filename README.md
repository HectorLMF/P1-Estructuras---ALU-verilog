...
------------------------
- If test expectations disagree with module behavior (notably `preprocess` / `compl1`), inspect `vcd/preprocess_tb.vcd` or `vcd/alu_tb.vcd` with GTKWave and consider whether to:
  - Modify `preprocess.v` control mapping (case table) or `muxAdd` input ordering; or
  - Update the testbench expectations to match the intended implemented behavior.
- Warnings during `iverilog` compilation about port width mismatches often indicate that module ports are connected in a different order or width than expected; double-check module port lists.
...
--------------------
If you want, I can:
- Adjust `preprocess.v` to match a target truth-table (you provide the desired mapping), then re-run tests.
- Add more checks to `testbench/alu_tb.v` (e.g., verify arithmetic outputs exactly match `sum4_v2` when Op selects the adder).
- Reduce or expand the signals written to VCD for faster, smaller waveforms.

---
README generated by the assistant during an interactive session; edit as needed.
